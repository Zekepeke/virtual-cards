...
### Example 1: Create and Send a Birthday Card

```typescript
// User creates a birthday card
const card = await createCard({
  occasion: 'birthday',
  recipientName: 'Sarah',
  message: 'Happy Birthday! Hope your day is as amazing as you are! 🎉',
  photo: uploadedImage,
  scheduledFor: new Date('2025-03-15T09:00:00'),
});

// Share the card link
const shareLink = `${APP_URL}/card/${card.id}`;
await sendSMS(recipientPhone, `You have a special card waiting! ${shareLink}`);
```

**Expected Output:** Recipient receives an SMS at 9:00 AM on March 15th with a link to an animated birthday card.

### Example 2: Collaborative Anniversary Card

```typescript
// Multiple friends contribute to one card
const groupCard = await createCollaborativeCard({
  occasion: 'anniversary',
  recipientCouple: ['John', 'Emily'],
  creatorId: userId,
});

// Each person adds their message
await addCollaboratorMessage(groupCard.id, {
  authorName: 'Mike',
  message: 'Congrats on 5 amazing years together!',
});

await addCollaboratorMessage(groupCard.id, {
  authorName: 'Lisa',
  message: 'You two are relationship goals! ❤️',
});

// Schedule delivery
await scheduleCard(groupCard.id, new Date('2025-04-20T18:00:00'));
```

**Expected Output:** John and Emily receive a card signed by multiple friends at 6:00 PM on their anniversary.

### Example 3: Gift Link Integration

```typescript
// Create a card with a gift link
const cardWithGift = await createCard({
  occasion: 'just_because',
  recipientName: 'Mom',
  message: 'Thanks for everything! Treating you to coffee ☕',
  giftLinks: [
    {
      type: 'venmo',
...
**Expected Output:** Recipient opens the card and sees a button to claim the gift via Venmo.
